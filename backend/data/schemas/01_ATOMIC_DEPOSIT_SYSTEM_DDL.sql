...
CREATE TABLE account (
  ...
  -- CHECK CONSTRAINTS
  CONSTRAINT ck_account_balance_non_negative CHECK (current_balance >= 0),
  CONSTRAINT ck_account_accrued_interest_non_negative CHECK (accrued_interest_not_posted >= 0),
...
CREATE TABLE deposit_insurance_calculation (
  ...
  CONSTRAINT ck_insured_uninsured_sum CHECK (
    ...
    AND (calculated_insured_amount + calculated_uninsured_amount - input_account_balance) > -0.01
  )
);
